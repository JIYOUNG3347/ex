...
class InitPublisher(Node):
    ...
    def timer_callback(self):
        ...
        point.time_from_start = rclpy.time.Duration(seconds=3.0).to_msg()
...
class CapstonJointController(Node):
    ...
    def listener_callback(self, msg):
        ...
        # Init Position
        init_position = [0.0, -1.57, 0.0, -1.57, 0.0]

        # Current position
        current_position = list(np.round(msg.actual.positions,3))

        # Tartget postion
        target_position = [float(self.policies[self.i][self.j]), 0.0, float(self.policies[self.i][self.j+1]), 0.0, 0.0, 0.0]
        
        # Max velocity
        max_vel = float(self.policies[self.i][4])
        ...
        if (target_position == current_position) and (self.j == 0):
            ...
            input('Press key to restart project...')
            print('Restart')
            self.i = self.i + 1
            self.j = 0
